# Model variant: Accelerated mRNA dynamics (K_tau = 100)
#
# Eliminates the response speed advantage of post-transcriptional over transcriptional
# CCR by multiplying both synthesis and degradation terms in dM1/dt and dM2/dt by K_tau.
# This shortens the mRNA response time from ln(2)/delta1 to ln(2)/(K_tau*delta1),
# so mRNA reaches steady state nearly instantaneously upon environmental changes.
# The corresponding mRNA terms in dc⁻/dt are also scaled by K_tau for mass conservation.
# All other equations (X1, X2, E1, E2, c) remain identical to the default model.
